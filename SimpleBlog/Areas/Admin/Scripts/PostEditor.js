$(document).ready(function() {
    ...
    $newTagName
        .keyup(function() { // determines whether the AddTag button should be enabled or disabled...
            if ($newTagName.val().trim().length > 3)
                $addTagButton.prop("disabled", false);
            else
                $addTagButton.prop("disabled", true);
        })
        .keydown(function(e) { // just determines whether the "enter" key (key # 13 I guess) is the one being pressed and prevents default actions and calls the function
            if (e.which != 13)
                return;
    ...
    function addTag(name) {
        ...
        $tagEditor.find(".tag-select > li.template").clone()
